...
function sendRaw(message: OutgoingMessage) {
...
socket.addEventListener("message", (event) => {
    ...
    if (msg.type === "opponent_disconnected") {
        showWarnToast(
            `Opponent disconnected. Waiting ${msg.timeout ?? ""}`.trim(),
        );
        return;
    }
    ...
    if (msg.type === "game_over") {
        ...
        if (msg.winner === authStore.getUser()?.username) {
            ...
            showDefeatToast(
                `Unlucky ${authStore.getUser()?.username},  ${msg.winner} defeated you! \n Reason: ${msg.reason}`,
            );
        }
        clearAllObservables();
        gameStore.reset();
...
export function sendRegistrationDetails(username: string, password: string) {
    sendRaw({ type: "register", username, password });
}
...
export function sendResumeMessage(token: string) {
    sendRaw({ type: "resume", sessionToken: token });
    resuming = true;
}
